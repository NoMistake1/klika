...
Tokens live in `src/app/globals.css` under `@theme`. Three decisions there are
load-bearing and were made against measured contrast ratios, not by eye:

- **No orange.** Buttons are dark navy (`primary`), light blue (`secondary`) and
  beige (`accent`), plus `cream` for dark surfaces and flat `outline` / `ghost`.
  Every fill carries navy or warm-white text at ≥9.6:1.
- **`--color-sand-ink` (#96774B)** is beige used as *ink*. Plain `--color-sand`
  on cream is 1.49:1 — invisible. The deep tone clears the 3:1 large-text bar on
  every light surface (cream 3.64, warm-white 4.10, light blue 3.44), while
  `--color-sand` itself is used for handwriting on navy (9.89:1). Caveat notes
  therefore take `tone="beige"` on light sections and `tone="beigeLight"` on
  navy ones.
...
**Placeholders — every image on the site.** Generated by
`node scripts/generate-placeholders.mjs`: branded, clearly labelled
"PLACEHOLDER" compositions. No stock photography is used anywhere — a random
hotel photo presented as Hotel Klika would misrepresent the business.

| Placeholder | Replace with |
| --- | --- |
| `public/images/**/*.png` (31 files) | Real photography, same filename + aspect ratio |
| `public/images/hero/poster.png` | The hero video's first frame, 1920×1080 |
| `public/video/` (empty) | `hero.webm` + `hero.mp4` — see `public/video/README.md` |
| `src/app/icon.png`, `apple-icon.png`, `opengraph-image.png` | Generated from the official logo |
| `src/components/ui/Logo.tsx` | The official logo SVG |

**The logo is a placeholder.** No official logo file was supplied, and the
reference screenshots must not be traced or used as production assets. The
wordmark is currently set in Lobster (the real brand display face — honest
typography, not an invented mark) and the monogram is a neutral stand-in.
`Logo.tsx` is the only file to change when the final SVG arrives.
...
- The hero poster is the real above-the-fold asset — first paint never waits on
  ...
  the poster in place.
- The map is a local illustration until clicked: no third-party request, script
  or cookie reaches Google unless the guest asks for it.
