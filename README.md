...
| `content/contact.ts` | Phones, e-mails, address, hours, billing | |
...
**Real:** the three fonts (Geist, Lobster, Caveat — genuine OFL releases,
subset to Latin + Latin Extended-A so Czech and German render from one file
each; licences ship in `src/assets/fonts/`).
...
- The map is a local illustration until clicked: no third-party request, script
  or cookie reaches OpenStreetMap unless the guest asks for it.
