...
function inputClasses(hasError: boolean): string {
  return cn(
    // Width containment for the field. min-w-0 lets it shrink to its column;
    // max-w-full caps it there. This alone contains the field on desktop, but
    // NOT in iOS Safari, where the native date control keeps an intrinsic width
    // regardless — that half is handled in globals.css, which resets the
    // control's appearance below the desktop breakpoint so width:100% finally
    // wins. box-sizing is border-box globally, so the padding stays inside.
    "min-h-11 w-full max-w-full min-w-0 rounded-lg border bg-navy/45 px-3.5 py-2 text-sm text-cream",
...
function BarField({
  ...
  return (
    // w-full + max-w-full + min-w-0 so this grid/flex child fills its column
    // yet may still shrink below the intrinsic width of the native date input
    // it wraps; without min-w-0 the column, not the field, is what overflows.
